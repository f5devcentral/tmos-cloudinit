            ...
            if hasattr(response, 'status_code'):
                if response.status_code == 200:
                    ...
                    if status_code == 200 and status == "OK":
                        ...
                        if 'errors' in json_resp['results']:
                            if json_resp['errors'][0].find(
                                    '/tm/sys/available. Service is unavailable.'
                            ) > 0:
                                LOG.error(
                                    'f5-declarative-onboarding can not progress due to system service error. restarty servics'
                                )
                                ...
                                LOG.debug(
                                    'f5-declarative-onboarding error %s - %s - %s',
                                    status_code, status,
                                    json_resp['errors'][0])
                                return False
                            time.sleep(1)
