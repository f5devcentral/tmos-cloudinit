                ...
                if response.status_code == 200:
                    ...
                    if status_code == 200 and status == "OK":
                        ...
                        if restarted_services:
                            LOG.error('system not responsive after service restart, aborting onboarding')
                            return False
                        LOG.error(
                            'f5-declarative-onboarding reporting system not responsive.. restarting services')
                        ...
                        tmos_onboard_utils.wait_for_icontrol()
                        LOG.debug(
                            'waiting for iControl LX to become available')
                        tmos_onboard_utils.wait_for_icontrollx()
                        LOG.debug('starting f5-declarative-onboarding watcher')
                        if not tmos_onboard_utils.do_declare():
                            LOG.error(
                                'f5-declarative-onboarding post after service restart failed'
                            )
                            return False
                    else:
                        if status_code > 399:
...
def clean():
    ...
    injected_already = subprocess.Popen(
        "cat /config/startup | grep " + DO_DELAYED_DEPLOYMENT_SCRIPT +
        " | wc -l",
    ...
    if injected_already == '1':
        subprocess.call([
            '/bin/sed', '-i',
            "/%s/d" % DO_DELAYED_DEPLOYMENT_SCRIPT, '/config/startup'
        ])
