...
def resolve_resources(rd_enabled=True,
    ...
    try:
        ...
        if 'fixed-address' in mgmt_data:
            ...
            if dhcpv4_options and 'fixed-address' in dhcpv4_options:
                LOG.info(
                    "overriding fixed-address from user-data supplied option to: %s",
                    dhcpv4_options['fixed-address'])
                m_ip = dhcpv4_options['fixed-address']
        if 'subnet-mask' in mgmt_data:
            m_nm = mgmt_data['subnet-mask']
            if dhcpv4_options and 'subnet-mask' in dhcpv4_options:
                LOG.info(
                    "overriding subnet-mask from user-data supplied option to: %s",
                    dhcpv4_options['subnet-mask'])
                m_nm = dhcpv4_options['subnet-mask']
        if 'interface-mtu' in mgmt_data:
            m_mtu = int(mgmt_data['interface-mtu'])
            if dhcpv4_options and 'interface-mtu' in dhcpv4_options:
                LOG.info(
                    "overriding interface-mtu from user-data supplied option to: %s",
                    dhcpv4_options['interface-mtu'])
                m_mtu = dhcpv4_options['interface-mtu']
        if 'routers' in mgmt_data:
            m_gw = mgmt_data['routers']
            if dhcpv4_options and 'routers' in dhcpv4_options:
                LOG.info(
                    "overriding routers from user-data supplied option to: %s",
                    dhcpv4_options['routers'])
                m_gw = dhcpv4_options['routers']
        if 'host-name' in mgmt_data:
            services['hostname'] = mgmt_data['host-name']
            if dhcpv4_options and 'host-name' in dhcpv4_options:
                LOG.info(
                    "overriding host-name from user-data supplied option to: %s",
                    dhcpv4_options['host-name'])
                services['hostname'] = dhcpv4_options['host-name']
        if 'domain-name' in mgmt_data:
            services['domainname'] = mgmt_data['domain-name']
            if dhcpv4_options and 'domain-name' in dhcpv4_options:
                LOG.info(
                    "overriding domain-name from user-data supplied option to: %s",
                    dhcpv4_options['domain-name'])
                services['domainname'] = dhcpv4_options['domain-name']
        if 'domain-name-servers' in mgmt_data:
            ...
            if dhcpv4_options and 'domain-name-servers' in dhcpv4_options:
                LOG.info(
                    "overriding domain-name-servers from user-data supplied option to: %s",
                    dhcpv4_options['domain-name-servers'])
                dns_servers = dhcpv4_options['domain-name-servers']
            if ',' in dns_servers:
        ...
        if 'ntp-servers' in mgmt_data:
            ...
            if dhcpv4_options and 'ntp-servers' in dhcpv4_options:
                LOG.info(
                    "overriding ntp-servers from user-data supplied option to: %s",
                    dhcpv4_options['ntp-servers'])
                ntp_servers = dhcpv4_options['ntp-servers']
            if ',' in ntp_servers:
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        create_onboard_artifacts(resources, do_enabled,
                                 ...
                                 ts_declaration, ts_declaration_url,
                                 as3_declaration, as3_declaration_url,
                                 post_onboard_commands)
