...
def create_pre_do_onboard():
    ...
    with open(onboard_script, 'w') as obs:
        ...
        obs.write("check_mcpd_up\n")
        obs.write("bigstart restart restjavad\n")
    ...
    if not is_startup_injected(DELAYED_DECLARATION_SCRIPT):
        ...
        with open('/config/startup', 'a+') as css:
            css.write("/usr/bin/env python %s %s &\n" %
                      (deploy_script_name, MODULE_NAME))
    LOG.info(
        'running declarations delayed deployment script %s', deploy_script_name)
    subprocess.call(['/usr/bin/env', 'python',
                     deploy_script_name, MODULE_NAME, '&'])
...
def onboard(trusted_sources, icontrollx_package_urls, do_enabled,
            ts_enabled, as3_enabled, post_onboard_enabled,
