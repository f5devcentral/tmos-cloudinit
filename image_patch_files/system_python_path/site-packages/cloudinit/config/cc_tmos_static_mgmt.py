...
def create_onboard_artifacts(mgmt_cidr, mgmt_gw, mgmt_mtu, hostname,
    ...
    with open(network_onboard_script, 'w') as mgmt_script:
        ...
        if hostname:
            mgmt_script.write("tmsh modify sys global-settings hostname %s\n" %
                              hostname)
            mgmt_script.write(
        ...
        if ntpservers:
            mgmt_script.write(
                ...
                " ".join(ntpservers))
            mgmt_script.write("tmsh modify sys ntp timezone %s\n" %
                              DEFAULT_TIMEZONE)
            mgmt_script.write(
...
def onboard(trusted_sources, icontrollx_package_urls,
            phone_home_url, phone_home_url_headers,
    ...
    if phone_home_url:
        tmos_onboard_utils.phone_home(phone_home_url, 
