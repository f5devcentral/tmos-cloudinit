...
def handle(name, cloud_config, cloud, log, args):
    ...
    if os.path.exists(VENDOR_DATA_RAW_FILE):
        ...
        if public_keys:
            ...
            root_password = 'root:' + ''.join([random.choice(string.ascii_letters + string.digits) for n in range(32)]) + '\n'
            try:
                util.subp(['chpasswd'], root_password)
            except Exception as e:
                LOG.error('failed to randomize passord for root user: %s', e)
            admin_password = 'admin:' + ''.join([random.choice(string.ascii_letters + string.digits) for n in range(32)]) + '\n'
            try:
