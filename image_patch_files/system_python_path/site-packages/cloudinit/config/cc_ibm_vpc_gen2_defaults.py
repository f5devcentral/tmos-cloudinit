...
def handle(name, cloud_config, cloud, log, args):
    ...
    # forcing hostname
    if "local-hostname" in cloud_config:
        LOG.debug('forcing localhost name to %s' % cloud_config['local-hostname'])
        tmos_onboard_utils.wait_for_mcpd()
        tmos_onboard_utils.run_cmd("tmsh modify sys global-settings hostname %s.local" % cloud_config['local-hostname'])
