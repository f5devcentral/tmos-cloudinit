...
def handle(name, cloud_config, cloud, log, args):
    ...
    if os.path.exists(VENDOR_DATA_RAW_FILE):
        ...
        if public_keys:
            ...
            root_password = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(32)]) + '\n'
            try:
                util.subp(['chpasswd', 'root'], root_password)
            except Exception as e:
                LOG.error('failed to randomize passord for root user: %s', e)
            admin_password = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(32)]) + '\n'
            try:
                util.subp(['chpasswd', 'admin'], admin_password)
            except Exception as e:
                LOG.error('failed to randomize passord for admin user: %s', e)
