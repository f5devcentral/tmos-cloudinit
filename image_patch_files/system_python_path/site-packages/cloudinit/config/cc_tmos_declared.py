...
def run_declarations_deffered():
    ...
    deploy_script_name = "%s/%s" % (
        os.path.dirname(os.path.realpath(__file__)), DELAYED_DECLARATION_SCRIPT)
    if not is_startup_injected(DELAYED_DECLARATION_SCRIPT):
        ...
        with open('/config/startup', 'a+') as css:
            css.write("/usr/bin/env python %s %s &\n" %
                      (deploy_script_name, MODULE_NAME))
    LOG.info(
        'running declaration delayed deployment script %s', deploy_script_name)
    subprocess.call(['/usr/bin/env', 'python',
                     deploy_script_name, MODULE_NAME, '&'])
...
def onboard(do_declaration, as3_declaration, ts_declaration, trusted_sources,
    ...
    if phone_home_url:
        tmos_onboard_utils.phone_home(
            phone_home_url, do_enabled, ts_enabled, as3_enabled, onboard_status, phone_home_url_verify_tls)
