...
def inject_tg_action_script(action):
    ...
    if not is_tg_action_injected(action, notify_script_name):
        LOG.info(
            'injecting tg action script %s into /config/failover/tg%s',
...
def onboard(do_declaration, do_declaration_url, as3_declaration,
            as3_declaration_url, ts_declaration, ts_declaration_url,
    ...
    if do_declaration:
        try:
            ...
            LOG.error(
                ...
                err)
            do_enabled = False
    elif do_declaration_url:
        try:
            tmos_onboard_utils.persist_do_declaration_from_url(
                    None, do_declaration_url)
        except Exception as err:
            LOG.error(
                'could not download f5-declarative-onboarding declaration: %s',
                err)
            do_enabled = False
    ...
    if ts_declaration:
        ...
        try:
            ...
            LOG.error(
                'could not download f5-telemetry-streaming declaration %s', err)
            ts_enabled = False
    ...
    if as3_declaration:
        ...
        try:
            ...
            LOG.error(
                'could not download f5-appsvcs-extension declaration %s', err)
            as3_enabled = False
...
def handle(name, cloud_config, cloud, log, args):
    ...
    if enabled:
        ...
        try:
            onboard(do_declaration, do_declaration_url, as3_declaration,
                    as3_declaration_url, ts_declaration, ts_declaration_url,
