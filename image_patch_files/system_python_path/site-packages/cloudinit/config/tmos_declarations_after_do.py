...
def clean():
    ...
    injected_already = subprocess.Popen(
        "cat /config/startup | grep " + DELAYED_DECLARATION_SCRIPT + " | wc -l",
        stdout=subprocess.PIPE, shell=True
    ).communicate()[0].replace('\n', '')
    if injected_already == '1':
        subprocess.call(['/bin/sed', '-i', "/%s/d" %
                         DELAYED_DECLARATION_SCRIPT, '/config/startup'])
...
def handle(name, cloud_config, cloud, log, args):
    ...
    try:
        if wait_for_do_to_complete():
            ...
            if tmos_onboard_utils.ts_declaration_persisted() and (not onboard_status == ERROR):
                ts_available = False
                for _ in range(5):
                ...
                if ts_available:
                    ...
                    if hasattr(response, 'status_code'):
                        if response.status_code == 204 or response.status_code == 200:
                            LOG.info('making initial f5-telemetry-streaming declaration')
                    ...
                    LOG.error('f5-telemetry-streaming never became available.. timeout...')
                    LOG.error('check /var/log/restnoded/restnoded.log for errors')
                    onboard_status = ERROR

            if tmos_onboard_utils.as3_declaration_persisted() and (not onboard_status == ERROR):
                as3_available = False
                for _ in range(5):
                    if tmos_onboard_utils.wait_for_rest_worker(
    ...
    if os.path.exists(CLOUD_CONFIG_FILE):
        ...
        if phone_home_url:
            tmos_onboard_utils.phone_home(
                phone_home_url, True, tmos_onboard_utils.ts_declaration_persisted(),
                tmos_onboard_utils.as3_declaration_persisted(),
                onboard_status, phone_home_url_verify_tls)
