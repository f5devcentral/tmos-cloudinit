...
def ts_declare():
    ...
    if is_rest_worker('/mgmt/shared/telemetry/declare') and os.path.isfile(TS_DECLARATION_FILE):
        ...
        LOG.error('f5-telemetry-streaming declaration failed %s - %s',
                  response.status_code, response.text)
        return False
    LOG.error('f5-telemetry-streaming worker not installed or declaration missing')
    return False
...
def get_ts_declaration():
    ...
    return requests.get(
        'http://localhost:8100/mgmt/shared/telemetry/declare',
        auth=('admin', '')
    )
    ...
    for pkg in os.listdir(PKG_INSTALL_DIR):
        if wait_for_mcpd() and wait_for_rest_worker('/mgmt/shared/iapp/package-management-tasks/'):
            ...
            if ext == 'rpm':
                ...
                if not int_check:
                    LOG.error(
                        '%s is not a valid RPM package or is corrupt', rpm_file_path)
                    continue
...
def phone_home(phone_home_url=None, do_enabled=False, ts_enabled=False,
               as3_enabled=False, status='ERROR', verify_tls=True, metadata={}):
    ...
    if phone_home_url:
        try:
            ...
            if wait_for_dns_resolution(fqdn, 120):
                ...
                if verify_tls:
                    resp = requests.post(phone_home_url, headers=headers, data=post_json)
                    resp.raise_for_status()
                else:
                    resp = requests.post(
                        phone_home_url, headers=headers, data=post_json, verify=False)
                    resp.raise_for_status()
                return True
        except Exception as err:
