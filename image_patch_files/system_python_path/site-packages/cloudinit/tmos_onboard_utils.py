...
DECLARATION_DOWNLOAD_RETRIES = 10
DECLARATION_DOWNLOAD_RETRY_DELAY = 10
...
def merge_dicts(dict1, dict2):
    """Merge two declared dictionarey objects"""
    return_dict = deepcopy(dict1)
    for key, value in dict2.iteritems():
        if isinstance(value, collections.Mapping):
            return_dict[key] = merge_dicts(return_dict.get(key, {}), value)
        else:
            return_dict[key] = deepcopy(dict2[key])
    return return_dict
...
def persist_do_declaration(declaration=None,
                           additional_declarations=None,
                           additional_declaration_url=None,
                           additional_declaration_headers=None,
                           jinja_download_template_dict=None):
    """Write the f5-declarative-onboarding declaration to file with additions from URL"""
    do_declaration_dir_exists()

    # download URL specified declaration
    additional_downloaded_declarations = None
    if additional_declaration_url:
        LOG.info('downloading f5-declarative-onboarding declaration from %s',
                 additional_declaration_url)
        downloaded_declaration_file = "%s/additional_declarations_url" % DO_DECLARATION_DIR
        download_tries = 1
        while (not os.path.exists(downloaded_declaration_file)) and (
                download_tries < DECLARATION_DOWNLOAD_RETRIES):
            if download_file(additional_declaration_url,
                             additional_declaration_headers,
                             downloaded_declaration_file):
                try:
                    with open(downloaded_declaration_file, 'r') as ad:
                        if jinja_download_template_dict:
                            if not TEMPLATING_ENABLED:
                                LOG.error(
                                    'jinja2 template dictionary supplied, but not support on platform. TMOS 15.1+ required.'
                                )
                            else:
                                downloaded_template = Template(ad.read())
                                rendered_declaration = downloaded_template.render(
                                    jinja_download_template_dict)
                                additional_downloaded_declarations = json.loads(
                                    rendered_declaration)
                        else:
                            additional_downloaded_declarations = json.load(ad)
                        # merge user-data declared Common items into downloaded declaration
                        if additional_declarations:
                            LOG.info(
                                'merging user-data f5-declarative-onboarding declaration into downloaded declaration'
                            )
                            additional_downloaded_declarations['Common'] = merge_dicts(
                                additional_downloaded_declarations['Common'],
                                additional_declarations['Common']
                            )
                            # old way was not a proper deep merge
                            # for key, value in additional_declarations[
                            #        'Common'].iteritems():
                            #    additional_downloaded_declarations['Common'][
                            #        key] = value
                        additional_declarations = additional_downloaded_declarations
                except Exception as ex:
                    LOG.error(
                        'f5-declarative-onboarding download declaration %s is not valid JSON: %s',
                        additional_declaration_url, ex)
            else:
                download_tries = download_tries + 1
                time.sleep(DECLARATION_DOWNLOAD_RETRY_DELAY)
                LOG.warning("retrying download from %s (%d:%d)",
                            additional_declaration_url, download_tries,
                            DECLARATION_DOWNLOAD_RETRIES)
    ...
    if os.path.isfile(DO_DECLARATION_FILE):
        if not additional_declarations:
            ...
            LOG.info(
                'using user-supplied f5-declarative-onboarding declarations')
    ...
    if declaration and additional_declarations:
        LOG.info(
            'merging user supplied f5-declarative-onboarding declarations into IaaS resolved declarations'
        )
        declaration['Common'] = merge_dicts(declaration['Common'], additional_declarations['Common'])
        # old method was not a deep copy or proper merge.. just overwrite
...
def download_as3_declaration(declaration_url=None,
    ...
    if declaration_url:
        ...
        LOG.info('downloading f5-appsvcs-extension declaration from %s',
                 declaration_url)
        download_tries = 1
        while (not os.path.exists(AS3_DECLARATION_FILE)) and (
                download_tries < DECLARATION_DOWNLOAD_RETRIES):
            if download_file(declaration_url, declaration_headers, AS3_DECLARATION_FILE):
                if jinja_download_template_dict:
                    if not TEMPLATING_ENABLED:
                        LOG.error(
                            'jinja2 template dictionary supplied, but not support on platform. TMOS 15.1+ required.'
                        ...
                        rendered_declaration = None
                        with open(AS3_DECLARATION_FILE, 'r') as ad:
                            downloaded_template = Template(ad.read())
                            rendered_declaration = downloaded_template.render(
                                jinja_download_template_dict)
                        if rendered_declaration:
                            with open(AS3_DECLARATION_FILE, 'w') as ad:
                                ad.write(rendered_declaration)
                return
            else:
                download_tries = download_tries + 1
                time.sleep(DECLARATION_DOWNLOAD_RETRY_DELAY)
                LOG.warning("retrying download from %s (%d:%d)",
                            declaration_url, download_tries,
                            DECLARATION_DOWNLOAD_RETRIES)
        LOG.error('f5-appsvcs-extension download declaration failed')
...
def download_ts_declaration(declaration_url=None,
    ...
    if declaration_url:
        ...
        LOG.info('downloading f5-telemetry-streaming declaration from %s',
                 declaration_url)
        download_tries = 1
        while (not os.path.exists(TS_DECLARATION_FILE)) and (
                download_tries < DECLARATION_DOWNLOAD_RETRIES):
            if download_file(declaration_url, declaration_headers, TS_DECLARATION_FILE):
                if jinja_download_template_dict:
                    if not TEMPLATING_ENABLED:
                        LOG.error(
                            'jinja2 template dictionary supplied, but not support on platform. TMOS 15.1+ required.'
                        ...
                        rendered_declaration = None
                        with open(TS_DECLARATION_FILE, 'r') as ad:
                            downloaded_template = Template(ad.read())
                            rendered_declaration = downloaded_template.render(
                                jinja_download_template_dict)
                        if rendered_declaration:
                            with open(TS_DECLARATION_FILE, 'w') as ad:
                                ad.write(rendered_declaration)
                return
            else:
                download_tries = download_tries + 1
                ...
                LOG.warning("retrying download from %s (%d:%d)",
                            declaration_url, download_tries,
                            DECLARATION_DOWNLOAD_RETRIES)
        LOG.error('f5-telemetry-streaming download declaration failed')
...
def download_file(source_url=None, headers={}, destination_file=None):
    """Download AT declarations to destination folder"""
    if source_url and destination_file:
        try:
                ...
                with open(tmp_file, 'wb') as out_file:
                    for chunk in resp.iter_content(chunk_size=8192):
                        if chunk:
                            ...
                            LOG.warning(
                                "received a null chunk when downloading %s" %
                                source_url)
                if os.stat(tmp_file).st_size == 0:
                    LOG.error("download %s returned zero bytes of content")
                    return False
                if os.path.isfile(tmp_file):
                    LOG.info("download complete.. moving %s to %s", tmp_file,
                             destination_file)
                    os.rename(tmp_file, destination_file)
                    return True
            else:
                LOG.error('DNS could not resolve host for %s', source_url)
            LOG.error('could not download %s to %s', source_url,
                      destination_file)
            return False
        except Exception as err:
    ...
    if phone_home_url:
        try:
            ...
            if wait_for_dns_resolution(fqdn, 120):
                ...
                if phone_home_headers:
                    for h in phone_home_headers:
                        headers[h] = phone_home_headers[h]
                post_data = {}
...
def create_tg_transition_data(traffic_group, action):
    """Creates the return data used by the tgactive, tgstandby, tgrefresh notifications"""
    if traffic_group:
        data = {
            'instance_id': None,
            'device_id': None,
        ...
        traffic_group = os.path.basename(traffic_group)
        instance_file = '/opt/cloud/data/instance-id'
        if os.path.exists(instance_file):
            with open(instance_file, 'r') as instf:
                data['instance_id'] = instf.read().replace('\n', '')
        url = 'http://localhost:8100/mgmt/shared/identified-devices/config/device-info'
        response = requests.get(url, auth=('admin', ''))
        if response.status_code < 400:
            response_json = response.json()
            if 'machineId' in response_json:
                data['device_id'] = response_json['machineId']
        url = 'http://localhost:8100/mgmt/tm/net/self'
        response = requests.get(url, auth=('admin', ''))
        if response.status_code < 400:
            response_json = response.json()
            if 'items' in response_json:
                for selfip in response_json['items']:
                    data['selfips'].append(selfip['address'])
        url = 'http://localhost:8100/mgmt/tm/ltm/virtual-address'
        response = requests.get(url, auth=('admin', ''))
        if response.status_code < 400:
            response_json = response.json()
            if 'items' in response_json:
                for va in response_json['items']:
                    if os.path.basename(va['trafficGroup']) == traffic_group:
                        data['virtual_addresses'].append({
                            'address':
                            va['address'],
                            'mask':
                            va['mask']
                        })
        url = 'http://localhost:8100/mgmt/tm/ltm/nat'
        response = requests.get(url, auth=('admin', ''))
        if response.status_code < 400:
            response_json = response.json()
            if 'items' in response_json:
                for nat in response_json['items']:
                    if os.path.basename(nat['trafficGroup']) == traffic_group:
                        data['nat_originating_addresses'].append(
                            nat['originatingAddress'])
        url = 'http://localhost:8100/mgmt/tm/ltm/snat-translation'
        response = requests.get(url, auth=('admin', ''))
        if response.status_code < 400:
            response_json = response.json()
            if 'items' in response_json:
                for snat in response_json['items']:
                    if os.path.basename(snat['trafficGroup']) == traffic_group:
                        data['snat_translation_addresses'].append(
                            snat['address'])
        return data


def send_tg_notification(url, traffic_group, action):
    """Sends the webhook notification for a tgactive, tgstandby, tgrefresh notifications"""
    if url:
        try:
            ...
            if wait_for_dns_resolution(fqdn, 120):
                headers = {'Content-Type': 'application/json'}
                post_data = create_tg_transition_data(traffic_group, action)
                post_json = json.dumps(post_data)
            ...
            LOG.error("could not notify tg state change: %s - %s", url, err)
    return False
