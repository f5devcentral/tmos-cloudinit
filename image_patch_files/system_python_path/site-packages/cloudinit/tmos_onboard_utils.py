...
PKG_INSTALL_DIR = '/var/lib/cloud/icontrollx_installs'
...
def persist_do_declaration(declaration=None, additional_declarations=None):
    ...
    if os.path.isfile(DO_DECLARATION_FILE):
        if not additional_declarations:
            LOG.info('found file injected declarations')
            with open(DO_DECLARATION_FILE, 'r') as ad:
                additional_declarations = yaml.safe_load(ad.read())   
        else:
            LOG.info(
                'using user-data or downloaded declarations instead of file injected declarations'
            )
    ...
    if declaration and additional_declarations:
        LOG.info(
            'additional declaration items overwriting IaaS resolved declarations'
        )
        for key, value in additional_declarations['Common'].iteritems():
            declaration['Common'][key] = value
    elif additional_declarations:
        LOG.info('no IaaS resolved declarations, using user declarations only')
...
def persist_do_declaration_from_url(declaration=None,
    ...
    if additional_declaration_url:
        LOG.info('downloading DO declaration from %s',
...
def download_as3_declaration(declaration_url):
    """Write the f5-appsvcs-extension declaration from URL"""
    as3_declaration_dir_exists()
    if os.path.isfile(AS3_DECLARATION_FILE):
        util.del_file(AS3_DECLARATION_FILE)
    download_file(declaration_url, AS3_DECLARATION_DIR, AS3_DECLARATION_FILE)
...
def download_ts_declaration(declaration_url):
        ...
        try:
            ...
            if wait_for_dns_resolution(fqdn, 120):
                if not destination_file:
                    destination_file = os.path.basename(source_url)
                tmp_file_name = destination_file + '.part'
                if os.path.isfile(tmp_file_name):
                    util.del_file(tmp_file_name)
                ...
                if not destination_file:
                    ...
                    if cont_disp:
                        cont_disp_fn = re.findall('filename=(.+)', cont_disp)
                        if cont_disp_fn > 0:
                            destination_file = cont_disp_fn[0]
                with open(tmp_file_name, 'wb') as out_file:
                    for chunk in resp.iter_content(chunk_size=8192):
                        if chunk:
                            out_file.write(chunk)
                if os.path.isfile(tmp_file_name):
                    destination_file = destination_folder + '/' + destination_file
                    util.copy(tmp_file_name, destination_file)
                    util.del_file(tmp_file_name)
                    return True
