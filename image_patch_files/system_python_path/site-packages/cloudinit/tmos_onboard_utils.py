...
LOG_FILE = '/var/log/f5-cloud-init.log'
...
ICONTROLLX_PACKAGE_MAX_RETRIES = 25
...
def wait_for_rest_worker(workerpath, timeout=None):
    ...
    while (end_time - time.time()) > 0:
        try:
            ...
            LOG.error("Retrying due to error: %s",  err)
...
def install_extensions(trusted_sources=True):
    ...
    for pkg in os.listdir(PKG_INSTALL_DIR):
        if wait_for_rest_worker('/mgmt/shared/iapp/package-management-tasks/'):
            ...
            if ext == 'rpm':
                ...
                if not int_check:
                    LOG.error('%s is not a valid RPM package or is corrupt', rpm_file_path)
                    continue
                ...
                rpm_installed = ""
                install_task_id = ""
                for _ in range(ICONTROLLX_PACKAGE_MAX_RETRIES):
                    try: 
                        install_task_id = create_install_task(rpm_file_path)
                        LOG.info('install task is: %s', install_task_id)
                        if install_task_id:
                        ...
                        LOG.error("Retrying install due to error: %s",  err)
                        time.sleep(ICONTROLLX_PACKAGE_INSTALL_DELAY)
            ...
            LOG.error("Wait for REST worker returned false")
    return True
