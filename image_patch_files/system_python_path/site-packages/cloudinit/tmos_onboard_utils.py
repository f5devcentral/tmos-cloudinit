...
DECLARATION_DOWNLOAD_RETRIES = 5
...
def download_as3_declaration(declaration_url=None):
    ...
    if declaration_url:
        ...
        while (not os.path.exists(AS3_DECLARATION_FILE)) and (
                download_tries < DECLARATION_DOWNLOAD_RETRIES):
            if download_file(declaration_url, AS3_DECLARATION_DIR,
            ...
            if wait_for_dns_resolution(fqdn, 120):
                ...
                if not destination_file:
                    ...
                    if cont_disp:
                        cont_disp_fn = re.findall('filename=(.+)', cont_disp)
                        if cont_disp_fn > 0:
                            destination_file = destination_folder + '/' + cont_disp_fn[
                                0]
                            LOG.warning(
                                'download destination file set by disposition header to: %s',
                                destination_file)
                        else:
                            destination_file = destination_folder + '/' + os.path.basename(
                                source_url)
                tmp_file = destination_file + '.part'
                if os.path.isfile(tmp_file):
                    util.del_file(tmp_file)
