...
def persist_do_declaration_from_url(declaration=None,
    ...
    if additional_declaration_url:
        ...
        declaration_file = "%s/additional_declarations" % DO_DECLARATION_DIR
        download_tries = 1
        while (not os.path.exists(declaration_file)) and (
                download_tries < DECLARATION_DOWNLOAD_RETRIES):
            if download_file(additional_declaration_url, declaration_file):
                try:
                    with open(declaration_file, 'r') as ad:
                        additional_declarations = json.load(ad)
                    util.del_file(declaration_file)
                    return persist_do_declaration(declaration,
        ...
        LOG.error('f5-declarative-onboarding download declaration failed')
